dgp_fun2 = function(ind_totalID = 100, 
  ...
  # 
  # # treated
  y_t = c_intercept + alpha_noise_t + ifelse(time<=5, 5+c_slope*time, 10+t_slope*time )
  # # counterfactual 1
  y_cf= c_intercept + 5 + alpha_noise_t + c_slope * time
  ## counterfactual 2
  ...
  y_cf3 = c_intercept + 5 + alpha_noise_t + ifelse(time<=5, ifelse(time == 5, 0, c_slope*time), (t_slope-1)*time )
  y_cf4 = c_intercept + 5 + alpha_noise_t + ifelse(time<=5, -c_slope*time, c_slope*time )
  y = ifelse(treat==0, y_c, y_t)
